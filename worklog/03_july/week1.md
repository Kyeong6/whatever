...
[시계열 데이터 저장](https://dsbook.tistory.com/347)
...
1. 예측값 확인
    - 주기(1시간)를 설정하면 관리자가 예측값을 보고 이후 행동을 진행할 수 있다
2. 이상치 확인
    - 예측값보다 확인하는 주기를 빠르게 설정(10분)해서 조기검출 진행

**목적에 대한 질문 사항**

1. 유량계는 센서에서 데이터 수집 또는 전송 간격이 어떻게되는지?(예를 들어 10분간격인지, 1초간격인지 등)
    - 유량계에서 보내주는 유량데이터의 간격은 '원하는대로' 설정가능함. 미니PC나 알고리즘 성능 등을 고민하며 적절한 데이터 수집간격 정하면 됨. 자료조사나 데이터 분석을 통해 적절한 시간간격을 설정하면 될 것으로 보임
    - 필요할 때 유량계 지시부쪽으로 콜을 주면 바로 데이터가 전송되어 오도록 설정도 가능함

2. 데이터 보유 기간은?

3. 이상치도 예측값으로 전달(알림)하는지? (실제 이상치일 때 전달인지, 예측했을 때 이상치가 나올 경우를 전달하는지 모호)