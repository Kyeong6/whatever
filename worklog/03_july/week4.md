...
**2-5. 이상치 기준 업데이트 스크립트 : build_criteria.py**

이상치 기준은 3개월마다 업데이트를 진행한다.

초기 3개월은 미리 넣어둔 데이터를 사용하고, 3개월이 지난 시점부터는 센서를 통해 얻은 데이터를 기반으로 이상치 기준을 변경하기 때문에 이에 따른 코드 진행이 필요하다.

**2-6. 스케줄링 스크립트 : scheduling.py**
